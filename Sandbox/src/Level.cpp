...
void Level::Init()
{
	m_TriangleTexture = Texture2D::Create(std::string(ASSET_PATH) + "/textures/Triangle.png");
	m_Player.LoadAssets();
...
void Level::OnRender()
{
	...
	//Backgorund
	Renderer2D::DrawQuad({ playerPos.x, 0.0f, -0.8f }, { 50.0f, 50.0f }, {0.3f, 0.3f, 0.3, 1.0f});
...
bool Level::CollisionTest()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		m_Points.emplace_back(playerTransformedVertices[i].x, playerTransformedVertices[i].y);
	}
